...
for fd in fds:
    ...
    cmd = f"ln -s {os.path.abspath(f'{jpeg_dir}/{fd}')} {save_jpeg_dir_all}"
    os.system(cmd)
    for i in range(1, cls_n+1):
        if not os.path.exists(f'{save_jpeg_dir}/{fd}_{i}'):
            cmd = f"ln -s {os.path.abspath(f'{jpeg_dir}/{fd}')} {save_jpeg_dir}/{fd}_{i}"
            os.system(cmd)
    ...
    for item in im_list:
        ...
        binary_map = (im > 0)
        mask_image = Image.fromarray((binary_map * 255).astype(np.uint8))
